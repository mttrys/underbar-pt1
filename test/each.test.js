...
describe('each()', () => {
  ...
  it('Additional Test: iterates every element of an array, passing that element, its corresponding index, and the entire array to the callback', () => {
    const arr = ['b', 'c', 'd', 'e'];
    let count = 0;
    _.each(arr, function(element, index, array) {
      expect(element).toEqual(array[index]);
  ...
  it('Addtional Test: iterates every element of an array-like object, passing that element, its corresponding index, and the entire array to the callback', () => {
    const arrayLikeObj = {
      length: 2,
      1: 'f',
      2: 'g'
